<script lang="ts">
  import { Moon, Clock, Loader } from 'lucide-svelte';
...
<div class="bg-white rounded-3xl border border-slate-100/90 p-5 shadow-[0_8px_30px_rgb(0,0,0,0.015)] flex flex-col justify-between h-full">
  <div>
    ...
    <div class="flex items-center gap-3 mb-5">
      ...
      <h3 class="text-sm font-black text-slate-800 tracking-tight">Tidur Malam</h3>
    </div>

    <!-- Inputs -->
    <div class="space-y-4">
      <div>
        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Waktu Tidur</label>
        <div class="relative">
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-2">
          <button
            type="button"
            onclick={() => doa = true}
            class="py-2 px-4 rounded-xl text-xs font-extrabold border transition-all cursor-pointer {doa === true ? 'bg-sky-50 border-sky-200 text-sky-700 shadow-xxs' : 'bg-white border-slate-100 text-slate-400 hover:bg-slate-50'}"
          >
            Ya
          </button>
          <button
